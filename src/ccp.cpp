...
void cp::cp_origin()
{
	...
	double* max_V_each_segment; 
	max_V_each_segment=(double*)malloc(time_series_len * sizeof(double));
	//int max_m_l_each_segment[time_series_len];
	int* max_m_l_each_segment;
	max_m_l_each_segment= (int*)malloc(time_series_len * sizeof(int));;
...
int main()
{
	...
	my_dist= (double*)malloc((ts_len * ts_len) * sizeof(double));

	for (i = 0;i < 40;i++)
	{
		for (j = 0;j < ts_d;j++)
		{
			my_time_series[i * ts_d + j] = (rand() % (1-0)) ;
		}
	}
	...
	for (i = 40;i < 80;i++)
	{
		for (j = 0;j < ts_d;j++)
		{
			my_time_series[i * ts_d + j] = (rand() % (3 - 2))+2;
		}
	}
	for(i=0;i<80;i++)
	{
		for(j=0;j<80;j++)
		{
			my_dist[80 * i + j] = sqrt((my_time_series[i* ts_d] - my_time_series[j* ts_d])* (my_time_series[i * ts_d] - my_time_series[j * ts_d])+ (my_time_series[i * ts_d] - my_time_series[j * ts_d]) * (my_time_series[i * ts_d] - my_time_series[j * ts_d])+ (my_time_series[i * ts_d+1] - my_time_series[j * ts_d+1]) * (my_time_series[i * ts_d+2] - my_time_series[j * ts_d+2]));
		}
	}
	//cp cp_1(my_time_series, ts_len, ts_d, 3, 0.05,true,NULL,NULL,NULL,NULL);
	//cp::cp(double* my_time_series, int my_time_series_len, int my_time_series_d, int my_min_segment_size, double my_pt, bool my_is_Euclidean_distance, double* my_dist, int my_block_size, int* my_change_point_set, int* my_change_point_num)
	...
	cp cp_1(my_time_series, ts_len, ts_d, 3, 0.05, true, my_dist,5, change_point_set, change_point_num);
